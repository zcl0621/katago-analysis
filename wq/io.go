...
func SaveCollection(nodes []*Node) (sgf string) {
...
func (self *Node) Save() string {
	return SaveCollection([]*Node{self}) // Not using self.GetRoot() since SaveCollection does.
}

func (self *Node) write_tree() (sgf string) {
...
// instead.
func Load(sgfstr string) (*Node, int, error) {
	...
	blackStep := strings.Count(sgfstr, ";B[")
	whiteStep := strings.Count(sgfstr, ";W[")
	step := blackStep + whiteStep
	if err != nil {
		return nil, 0, err
	}
...
func LoadCollection(sgfstr string) ([]*Node, error) {
