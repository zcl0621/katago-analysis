...
// 0,1,2,3 ... 18 to abcdefghjklmnopqrstuvwxyz
func XYToGTP(x, y int) string {
	x++
	y++
	letter := fmt.Sprintf("%c", 'A'+x-1)
	if x >= 9 {
		letter = fmt.Sprintf("%c", 'A'+x) // 如果x大于等于9，需要跳过字母I
	}
	if letter == "@" {
		fmt.Println(letter)
	}
	number := fmt.Sprintf("%d", y)
