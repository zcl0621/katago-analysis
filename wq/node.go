...
func (self *Node) WriteTo() (node string) {
...
func (self *Node) GetLastMove() (x int, y int, color string, onBoard bool) {
	lastChild := self.LastChild()
	var lastMove []string
	if self.LastColor() == "B" {
		color = "W"
	} else if self.LastColor() == "W" {
		color = "B"
	}
	lastMove = lastChild.AllValues(color)
	if len(lastMove) == 1 {
		x, y, onBoard = ParsePoint(lastMove[0], self.Board().Size)
	}
	return x, y, color, onBoard
}
